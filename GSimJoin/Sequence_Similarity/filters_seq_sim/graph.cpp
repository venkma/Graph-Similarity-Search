...
void Graph:: readGraph(ifstream &inp)
{
	...
	for(unsigned ind=0; ind<vertexCount; ind++)
		vid_to_ind[vertices[ind].vid] = ind; // mapping vertex-id to its index
...
void Graph::computeMinHashes()
{
	static MinHash minhash;
	if(shingles.size()==0)
		return;
	minhash.computeMinHash(shingles, minhashes);
}

// for sorting edge-list of each vertex
