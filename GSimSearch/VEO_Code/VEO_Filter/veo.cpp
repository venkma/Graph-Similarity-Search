...
void VEO:: ranking(vector<Graph> &graph_dataset, Graph &query_graph) 
{
	...
	// traversing the vertex-set of query graph
	for(int vtx_ind = 0; vtx_ind < query_graph.vertices.size(); vtx_ind++)
	{
		pair<unsigned, unsigned> vtx_pair = make_pair(query_graph.vertices[vtx_ind], query_graph.vertices[vtx_ind]);
		if(rank.find(vtx_pair) != rank.end())
			rank[vtx_pair]++;
		else
			rank[vtx_pair] = 0;
	}
	// traversing the edge-set of query graph
	for(int edge_ind = 0; edge_ind < query_graph.edges.size(); edge_ind++)
	{
		pair<unsigned, unsigned> edge_pair;
		if(query_graph.edges[edge_ind].first > query_graph.edges[edge_ind].second)
			edge_pair = make_pair(query_graph.edges[edge_ind].second, query_graph.edges[edge_ind].first);
		else
			edge_pair = make_pair(query_graph.edges[edge_ind].first, query_graph.edges[edge_ind].second);
		if(rank.find(edge_pair) != rank.end())
			rank[edge_pair]++;
		else
			rank[edge_pair] = 0;
...
void VEO:: buildPrefix(vector<Graph> &graph_dataset, Graph &query_graph, int mode, bool isBucket, int no_of_buckets)
{
	...
	// For query graph
	unsigned prefixLength = 0;	
	double qg_size = query_graph.vertexCount + query_graph.edgeCount; // size of query graph 

	if(mode == 3)
	{
		double invUbound = (double)1.0/ubound; // inverse of ubound
		prefixLength = 1 +(unsigned)(ceil((qg_size*(double)(1.0-invUbound)))); // Prefix Length
	}
	else
	{
		prefixLength = qg_size;
	}
		
	// Constructing the rank-list 

	// Putting vertex and edge ranks of query graph together
	vector<unsigned> graph_ranks;
	for(int vtx_ind = 0; vtx_ind < query_graph.vertices.size(); vtx_ind++)
	{
		pair<unsigned, unsigned> vtx_pair = make_pair(query_graph.vertices[vtx_ind], query_graph.vertices[vtx_ind]);
	...
	// sort the ranks of the query graph in descending order  
	sort(graph_ranks.begin(), graph_ranks.end(), greater <>());

	// crop query graph's rank-list upto prefix-length
	for(int pref = 0; pref < prefixLength; pref++)
		rankList_qg.push_back(graph_ranks[pref]);

	if(isBucket)
	{
		bucket_qg.resize(no_of_buckets);
		for(int buck_ind = 0; buck_ind < no_of_buckets; buck_ind++)
			bucket_qg[buck_ind].resize(qg_size, 0);
		vector<unsigned> sumBucket(no_of_buckets, 0);
		// traversing graph's rank-list in ascending order 
		for(int grank_ind = qg_size-1; grank_ind >= 0; grank_ind--)
		{
			sumBucket[graph_ranks[grank_ind]%no_of_buckets]++;
			for(int buck_ind = 0; buck_ind < no_of_buckets; buck_ind++)
			{
				bucket_qg[buck_ind][grank_ind] = sumBucket[buck_ind];
			}
		}
	}
}
...
void VEO:: index(vector<Graph> &graph_dataset, Graph &query_graph, int mode, bool isBucket, int no_of_buckets)
{	
	ranking(graph_dataset, query_graph); // ranking vertices and edges together
	buildPrefix(graph_dataset, query_graph, mode, isBucket, no_of_buckets);
}

// Applies index filter on Graph g1 and query_graph
bool VEO:: indexFilter( Graph &g1, Graph &query_graph, int index1, int mode, bool isBucket, int no_of_buckets, long unsigned &indexCount, long unsigned &partitionCount, double threshold)
{
	unsigned size1 = g1.vertexCount + g1.edgeCount; // Size of Graph g2
	unsigned size_qg = query_graph.vertexCount + query_graph.edgeCount; // Size of Graph g1
	...
	if(mode == 3)
	{
		prefix1 = rankList[index1].size(); // prefix-length of graph g2
		prefix_qg = rankList_qg.size(); // prefix-length of graph g1
	}
	if(mode == 4)
	{
		unsigned common = (unsigned)floor(((double)(threshold/200.0))*(double)(size1+size_qg));
		prefix1 = size1 - common + 1; // prefix-length of graph g1
		prefix_qg = size_qg - common + 1; // prefix-length of graph query graph
	}
	...
	while(start1 < prefix1 && start_qg < prefix_qg)
	{
		if(rankList[index1][start1] == rankList_qg[start_qg] && isBucket)
		{
			out = false;
			start1++;
			start_qg++;
		...
		else if(rankList[index1][start1] == rankList_qg[start_qg] && !isBucket)
		{
			out = false;
			break;
		}
		else if(rankList[index1][start1] > rankList_qg[start_qg])
			start1++;
		else
			start_qg++;
	}
	...
	if(isBucket)
	{
		for(int i = 0; i < no_of_buckets; i++)
			commonTotal += (long double)min(bucket[index1][i][start1], bucket_qg[i][start_qg]);

		long double sizeSum = (long double)(g1.vertexCount + g1.edgeCount + query_graph.vertexCount + query_graph.edgeCount);
...
// MisMatching Filter
bool VEO:: mismatchingFilter(Graph &g1, Graph &query_graph, double &common, double threshold)
{
	int index1 = 0;
	int index_qg = 0;
	unsigned degreeSum_1_qg = 0;
	unsigned degreeSum_qg_1 = 0;
	while(index1 < g1.vertexCount && index_qg < query_graph.vertexCount)
	{
		if(g1.vertices[index1] == query_graph.vertices[index_qg])
		{
			common++;
			index1++;
			index_qg++;
		}
		else if(g1.vertices[index1] > query_graph.vertices[index_qg])
		{
			degreeSum_qg_1 += query_graph.degrees[query_graph.vid_to_ind[query_graph.vertices[index_qg]]];
			index_qg++;
		}
		else
		{
			degreeSum_1_qg += g1.degrees[g1.vid_to_ind[g1.vertices[index1]]];
			index1++;
		}
	}

	double ES = (double)min((g1.edgeCount - degreeSum_1_qg), (query_graph.edgeCount - degreeSum_qg_1) );
	ES = max(0.0,ES);
	double simEstimate = (double)(200.0*(double)((common + ES)/(double)(g1.edgeCount + g1.vertexCount + query_graph.edgeCount + query_graph.vertexCount)));
